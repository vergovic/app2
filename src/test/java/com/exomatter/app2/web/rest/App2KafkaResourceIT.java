...
import com.exomatter.app2.config.EmbeddedKafka;
import com.exomatter.app2.config.KafkaSseConsumer;
import com.exomatter.app2.config.KafkaSseProducer;
...
class App2KafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/app-2-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/app-2-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/app-2-kafka/unregister"));
                return;
            }
        }
